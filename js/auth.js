const Auth = (() => {
    ...
    const presence = uid => {
        const go = v => db.collection('users').doc(uid).update({ online: v, lastSeen: firebase.firestore.FieldValue.serverTimestamp() }).catch(() => {});
    ...
    const register = async (email, pw, name, username) => {
        ...
        _u = c.user;
        _p = { name, username: username.toLowerCase(), email, bio: '', online: true, lastSeen: firebase.firestore.FieldValue.serverTimestamp(), createdAt: firebase.firestore.FieldValue.serverTimestamp() };
    ...
    const logout = async () => {
        if (_u) await db.collection('users').doc(_u.uid).update({ online: false, lastSeen: firebase.firestore.FieldValue.serverTimestamp() }).catch(() => {});
        Crypto.clearCache();
        await auth.signOut();
        _u = _p = _kp = _pub = null;
    };
    ...
    return { init, user, profile, keyPair, pubKey, needsOnboarding, register, login, google, logout, update, saveOnboarding, checkUsername, regenKeys, errMsg };
})();
